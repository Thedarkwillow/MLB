...
export default function BoardPage() {
  ...
  const filtered = useMemo(() => {
    return rows.filter((row) => {
      if (oddsFilter !== "all" && (row.odds_type ?? "standard") !== oddsFilter) return false;
      if (statFilter && row.stat_type_norm !== statFilter) return false;
      return true;
    });
  }, [rows, oddsFilter, statFilter]);

  const statOptions = useMemo(() => {
    return Array.from(new Set(rows.map((r) => r.stat_type_norm))).sort();
  }, [rows]);

  return (
    <div style={{ display: "grid", gap: 16 }}>
      <h1 style={{ margin: 0, fontSize: 28 }}>Board</h1>

      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        <select value={oddsFilter} onChange={(e) => setOddsFilter(e.target.value)} style={selectStyle}>
      ...
      {loading ? (
        ...
        <Table
          rows={filtered}
          emptyText="No board rows yet."
          columns={[
            { key: "player", header: "Player", render: (r) => r.player_name },
            { key: "team", header: "Team", render: (r) => r.player_team ?? "—" },
            { key: "opp", header: "Opponent", render: (r) => r.opponent_team ?? "—" },
            { key: "stat", header: "Stat", render: (r) => r.stat_type_norm },
            { key: "line", header: "Line", render: (r) => (r.line ?? "—") as any },
            { key: "class", header: "Class", render: (r) => r.odds_type ?? "standard" },
            { key: "status", header: "Join", render: (r) => r.join_status },
          ]}
        />
      )}
    </div>
